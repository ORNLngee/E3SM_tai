...
contains
  ...
  subroutine CNSeasonDecidPhenology (num_soilp, filter_soilp, cnstate_vars)
    ...
    associate(                                                                                             &
         ...
         needleleaf                          =>    veg_vp%needleleaf                                 , & ! Input:  [real(r8)  (:)   ]  woody lifeform flag (0 = non-woody, 1 = tree, 2 = shrub)
         climatezone                         =>    veg_vp%climatezone                                , & ! Input:  [real(r8)  (:)   ]  woody lifeform flag (0 = non-woody, 1 = tree, 2 = shrub)
      ...
      do fp = 1,num_soilp
         ...
         if (season_decid(ivt(p)) == 1._r8) then
            ...
            if (dormant_flag(p) == 1.0_r8) then
               ...
               if (woody(ivt(p)) == 1 .and. &               ! tree
                   climatezone(ivt(p)) == 3 .and. &         ! boreal
                   season_decid(ivt(p)) == 1 .and. &        ! deciduous
                   needleleaf(ivt(p)) == 1) then            ! needleleaf
                 !forcing
               ...
               elseif (woody(ivt(p)) == 2 .and. &               ! shrub
                       climatezone(ivt(p)) == 3 .and. &         ! boreal
                       season_decid(ivt(p)) == 1 .and. &        ! (season) deciduous
                       needleleaf(ivt(p)) == 0) then            ! broadleaf
